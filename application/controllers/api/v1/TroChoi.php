...
class Product extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        define("_TRO_NAY_HAY_NE_", "Hello world.");
        define("_TRO_NAY_DO_OM_", "Hello world.");
        define("_BUON_NGU_QUA_", "Hello world.");
        define("_DI_NGU_THOI_", "Hello world.");
    }
    ...
    public function giaiTriNgayChuNhat()
    {
        $troChoi = $this->security->xss_clean(trim($this->input->post('troChoi')));
        $thongTinTroChoi = array(
            'tenTroChoi' => $this->security->xss_clean(trim($this->input->post('tenTroChoi'))),
            'kiHieuTroChoi' => taoKiHieu($this->security->xss_clean(trim($this->input->post('tenTroChoi'))))
        );
        if (kiemTraTroChoi($thongTinTroChoi))
        {
            $ketQua = $this->troChoi->chonTroChoi($troChoi);
            if ($ketQua === _TRO_NAY_HAY_NE_)
            {
                $ketQua1 = $this->troChoi->chayTroChoi($troChoi, $thongTinTroChoi);
                if ($ketQua1 === _TRO_NAY_DO_OM_)
                {
                    $ketQua2 = $this->troChoi->tatTroChoi($troChoi);
                    if ($ketQua2 !== _BUON_NGU_QUA_)
                    {
                        $bayGioTinhSao = array(
                            'tinhTrang' => _DI_NGU_THOI_
                        );
                    }
                    else
                    {
